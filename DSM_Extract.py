...
def preprocess(img):
    w, h = img.size
    npdata = np.zeros((h, w), dtype=np.float)
    for i in range(h):
      for j in range(w):
        npdata[i][j] = img.getpixel((i,j)) # 获取像素值
    return npdata
#####在原栅格图像周围加一圈并返回
def AddRound(npgrid):
...
def Cacdxdy(npgrid,sizex,sizey):
    zbc=AddRound(npgrid)
    dx=((zbc[1:-1,:-2])-(zbc[1:-1,2:]))/sizex/2 # z值认为没有缩放过，栅格大小为0.5m
...
def Drawgrid(judge,pre=[],A=[],strs=""):
    x0 = 586243.1560000000
    y0 = 2568280.9649999999
    n = len(pre)
